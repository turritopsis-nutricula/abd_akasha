...
void btHfFluid::setFluidHeight (int x, int y, btScalar height)
{
	...
	//m_aabbMax.setY( m_aabbMax.getY() < height? height : m_aabbMax.getY() );
}
...
void btHfFluid::getAabbForColumn (int i, int j, btVector3& aabbMin, btVector3& aabbMax) const
{
...
void btHfFluid::foreachGroundTriangle(btTriangleCallback* callback,const btVector3& aabbMin,const btVector3& aabbMax) const
{
	btVector3 verts[3];
...
void btHfFluid::foreachFluidColumn (btHfFluidColumnCallback* callback, const btVector3& aabbMin, const btVector3& aabbMax) const
{
	btScalar minX, minZ, maxX, maxZ;
	int startNodeX, startNodeZ, endNodeX, endNodeZ;
...
btHfFluidColumnRigidBodyCallback::btHfFluidColumnRigidBodyCallback (
		btRigidBody const* rigidBody, btIDebugDraw* debugDraw, btScalar density, btScalar floatyness)
{
	m_rigidBody = rigidBody;
	assert(rigidBody->getCollisionShape()->getUserPointer());

	m_buoyantParameter =
		&BuoyantAuhoizer::getParameter(
				*(rigidBody->getCollisionShape())
				); ///(btHfFluidBuoyantConvexShape*)rigidBody->getCollisionShape();
	m_debugDraw = debugDraw;
	m_rigidBody->getAabb (m_aabbMin, m_aabbMax);
	m_volume = btScalar(0.0f);
	m_density = density;
	m_floatyness = floatyness;
	m_numVoxels = m_buoyantParameter->numVoxels_;
	...
	{
		btVector3 p = m_buoyantParameter->voxelPositions_[i];
...
bool btHfFluidColumnRigidBodyCallback::processColumn (btHfFluid const* fluid, int w, int l)
{
	...
	btScalar const dt(1.0f/60.0f);
	btScalar const& water_density = fluid->getDensity();

	btScalar columnVolume = btScalar(0.0f);
	m_resultImpluse.setZero();
	m_resultTorqueImpluse.setZero();

	for (int i = 0; i < m_buoyantParameter->numVoxels_; ++i)
	{
		...
		if (sphereVsAABB(
					columnAabbMin,
					columnAabbMax,
					m_voxelPositionsXformed[i],
					m_buoyantParameter->radius_))
		{
			m_voxelSubmerged[i] = true;
			btScalar voxelVolume = m_buoyantParameter->volumePerVoxel_;
...
// 				btScalar massDisplacedWater = voxelVolume * m_density * m_floatyness;
				btScalar massDisplacedWater = voxelVolume * water_density;
				...
				m_resultImpluse + = impulseVec;
#else
				// 				m_rigidBody->applyImpulse (impulseVec, relative_position);
				m_resultImpluse += impulseVec;
				m_resultTorqueImpluse += relative_position.cross(impulseVec*m_rigidBody->getLinearFactor());
#endif
			}
		}
	}
