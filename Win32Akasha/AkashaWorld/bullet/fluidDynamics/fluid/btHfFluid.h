...
#include "../buoyant/BuoyantParameter.hpp"
...
class btHfFluid : public btCollisionObject
{
	public:
		...
		void getAabbForColumn (int x, int y, btVector3& aabbMin, btVector3& aabbMax) const;
		...
		void adjustAabb()
		{
			btScalar max = -BT_LARGE_FLOAT;
			for (int i=0; i< getNumNodesLength()*getNumNodesWidth(); i++)
				max = max < m_eta[i] ? m_eta[i] : max;
			m_aabbMax.setY( max );
		};
		...
		class btHfFluidColumnCallback
		{
			public:
				...
				virtual bool processColumn (btHfFluid const* fluid, int w, int l)
				{
					return true; // keep going
				}
		};

		void foreachFluidColumn (btHfFluidColumnCallback* callback, const btVector3& aabbMin, const btVector3& aabbMax) const;
	...
	public:
		...
		void setMu(btScalar const& m)
		{
			m_mu = m;
		}
		btScalar const& getMu() const
		{
			return m_mu;
		}
		void setDensity(btScalar const& d)
		{
			m_density = d;
		}
		btScalar const& getDensity() const
		{
			return m_density;
		}
};
...
{
	protected:
		btRigidBody const* m_rigidBody;
		akasha::bullet::buoyant::BuoyantParameter const* m_buoyantParameter;
	...
	public:
		...
		btVector3 m_resultImpluse;
		btVector3 m_resultTorqueImpluse;
	public:
		btHfFluidColumnRigidBodyCallback (
				...
				btScalar floatyness);
		~btHfFluidColumnRigidBodyCallback ();
		bool processColumn (btHfFluid const* fluid, int w, int l) override;
